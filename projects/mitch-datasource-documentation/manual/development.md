...
* `semantic-release` to automate the release process to NPM, and creating git releases. Isn't actually an installed package, but used as part of the CI/CD CircleCI configuration as a CLI tool (called through node `npx` command). See the root `.circleci/config.yml` & `.circleci/notes.txt` for more details
* `netlify-cli` to automate the release process for the static HTML documentation generated by `esdoc` to `netlify` static website hosting service. Isn't actually an installed package, but used as part of the CI/CD CircleCI configuration as a CLI tool (called through node `npx` command). See the root `.circleci/config.yml` & `.circleci/notes.txt` for more details
