...
-   `CircleCI` For CI/CD pipeline to build, tests & publish the package.
-   `GitHub` To host the source code
-   `NPM Registry` To upload the node package and make it available as a node package
-   `Netlify` To host the static HTML documentation generated by `esdoc`
-   `Dependabot` To automatically create Pull Requests to update dependencies
...
-   `commitizen` lets you make commits in a certain format. Useful in combination with `semantic-release` which relies on commit messages being under a certain format. See `commit` script of the main repository scripts.
-   `husky` lets you add hooks to commits, i.e. run scripts when commits occurs.
-   `commitlint` ensures commits is in a certain format. If you try make a commit that breaks the format, the combination of `commitlint` and `husky` would stop you.
...
-   `remark-cli` which uses `remark-lint` to lint and fix markdown files (in the documentation project)
...
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| `install`           | Performs an `npm install` for all projects inside the `projects` folder.                                                        |
| `update`            | Performs an `npm update` for all projects inside the `projects` folder.                                                         |
| `exec-all-projects` | Performs a specified command for all projects inside the `projects` folder. E.g. `npm run exec-all-projects -- "npm outdated"`. |

### Main Scripts

| Script Name  | Description                                                                                                 |
| ------------ | ----------------------------------------------------------------------------------------------------------- |
| `dev`        | Lint, build/compile the code and docs using development settings. Does not include tests, as it takes time. |
| `prd`        | Lint, build/compile the code and docs using production settings. Does not include tests, as it takes time.  |
| `test`       | Run tests.                                                                                                  |
...
| `storybook`  | Runs the storybook applications. For manual integration tests with client-side frameworks.                  |
...
| `react-storybook`      | Runs the storybook application for React. Used For manual integration tests with React.                    |
| `vue-storybook`        | Runs the storybook application for Vue. Used For manual integration tests with React.                      |
...
Generates the HTML documentation based on:

-   Source code in the `mitch-datasource` project, and
-   The markdown files in the `manual` folder.

The built HTML documentation files will reside in the `docs` folder.

**Main Scripts**

| Script Name   | Description                                                                                    |
| ------------- | ---------------------------------------------------------------------------------------------- |
| `lint-md`     | Lints the markdown files to ensure it follows a certain style.                                 |
| `lint-md-fix` | Lints & fixes (whem possible) the markdown files to ensure it follows a certain style.         |
